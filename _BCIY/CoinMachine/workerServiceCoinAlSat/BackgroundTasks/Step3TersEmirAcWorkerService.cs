		...
		private async Task<int> TersEmirAcTRAILING()
		{
			...
			foreach (var emir in yeniEmirler)
			{
				try
				{
					...
					var kapatEmri = await BinanceHelper.Order(symbol: emir.Symbol.Replace(".P", ""), emirtipi: tersTipi, quantity: emir.ExecutedQuantity.ToString().Replace(",", "."), amount: emir.ClosePrice.Value.ToString().Replace(",", "."), StopPrice : emir.StopPrice.Value.ToString().Replace(",", "."),  type: "STOP_MARKET", orderID: emir.BinanceCloseOrderId);
